...
class InMemoryStateRepository:
    ...
    async def upsert_memory(self, memory: MemoryRecord) -> MemoryRecord:
        for existing in self.memories.values():
            if (existing.user_id, existing.memory_type, existing.normalized_content) == (
...
class PostgresStateRepository:
            ...
            async with conn.transaction():
                await conn.execute(
                    ...
                    (payload["id"], payload["user"]["id"], json.dumps(payload["user"]),
                     json.dumps(payload.get("metadata", {})), payload["created_at"], payload["updated_at"]),
                )
    ...
    async def get_conversation(self, conversation_id: str, user_id: str) -> dict[str, Any] | None:
        ...
        return {
            "id": row["id"], "user": row["user_snapshot"],
    ...
    async def _replace_messages(self, conn, payload: dict[str, Any]) -> None:
        for sequence_no, message in enumerate(payload.get("messages", [])):
            await conn.execute(
                ...
                (payload["id"], sequence_no, message["role"], message["content"],
                 json.dumps(message, default=str), message.get("timestamp")),
            )

    async def delete_conversation(self, conversation_id: str, user_id: str) -> bool:
        async with await self._connect() as conn:
            result = await conn.execute(
                "DELETE FROM agent_state.conversations WHERE id=%s AND user_id=%s", (conversation_id, user_id)
            )
        ...
        async with await self._connect() as conn:
            rows = await (await conn.execute(
                """SELECT id FROM agent_state.conversations WHERE user_id=%s
                   ORDER BY updated_at DESC LIMIT %s OFFSET %s""", (user_id, limit, offset)
            )).fetchall()
        result = []
        for row in rows:
    ...
    async def get_memory(self, memory_id: str, user_id: str) -> MemoryRecord | None:
        async with await self._connect() as conn:
            row = await (await conn.execute(
                "SELECT * FROM agent_state.memories WHERE id=%s AND user_id=%s", (memory_id, user_id)
            )).fetchone()
    ...
    async def list_memories(self, user_id: str, statuses: set[MemoryStatus] | None = None, *, include_global: bool = False) -> list[MemoryRecord]:
        ...
        sql += " AND (expires_at IS NULL OR expires_at > now()) ORDER BY updated_at DESC"
        async with await self._connect() as conn:
    ...
    async def delete_memory(self, memory_id: str, user_id: str) -> bool:
        async with await self._connect() as conn:
            result = await conn.execute("DELETE FROM agent_state.memories WHERE id=%s AND user_id=%s", (memory_id, user_id))
            return result.rowcount == 1
