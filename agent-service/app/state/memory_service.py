...
class MemoryService:
    def __init__(
        self, repository: StateRepository, embedder: Embedder | None = None,
        *, max_active_per_user: int = 500, retention_days: int = 90,
    ) -> None:
        ...
        *,
        ...
        duplicate = next(
            (item for item in existing if item.memory_type == memory_type
             and item.normalized_content == normalized), None
        )
    ...
    async def reject(self, user_id: str, memory_id: str) -> MemoryRecord | None:
        memory = await self.repository.set_memory_status(memory_id, user_id, MemoryStatus.REJECTED)
        if memory:
            await self.repository.add_memory_event(MemoryEvent(memory.id, user_id, "rejected"))
        return memory
    ...
    async def delete(self, user_id: str, memory_id: str) -> bool:
        ...
        if not memory:
            return False
        await self.repository.add_memory_event(MemoryEvent(memory.id, user_id, "deleted"))
    ...
    async def create_tool_candidate(
        ...
        return await self.create_candidate(
            user_id, sanitize_memory_content(question), MemoryType.TOOL_PATTERN,
            source="tool_usage", tool_name=tool_name, tool_args=args,
