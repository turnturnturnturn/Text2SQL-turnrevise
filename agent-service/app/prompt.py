...
Rules:
1. A question asking for a database fact, amount, count, average, list, ranking, grouping, or time-based statistic MUST call search_schema_knowledge and then safe_read_sql in the same request. Do not answer such questions from memory or from example values. Never end a data-query turn immediately after search_schema_knowledge.
